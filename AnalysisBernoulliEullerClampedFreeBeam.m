...
for j = 2:10
    ...
    while betaxl(j) == betaxl(j-1)
        x0 = x0 + 2;
        betaxl(j) = fzero('CharEqClampedFreeBeam',[x0]);
    end
end
...
for j = 2:10
    C13(j) = cosh(betaxl(j))-cos(betaxl(j))+(sin(betaxl(j))-sinh(betaxl(j)))*(cos(betaxl(j))+cosh(betaxl(j)))/(sin(betaxl(j))+sinh(betaxl(j)));
end
